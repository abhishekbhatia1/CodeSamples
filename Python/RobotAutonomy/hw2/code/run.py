...
def main(robot, planning_env, planner):
    ...
    planning_env.SetGoalParameters(goal_config, 0.2)
    startTime = time.time()
    plan = planner.Plan(start_config, goal_config)
    totalTime = time.time()-startTime
    distPlan = 0
    for i in range(0,len(plan)-1):
        distPlan += numpy.linalg.norm(numpy.array(plan[i+1])-numpy.array(plan[i]))
    plan_short = planning_env.ShortenPath(plan)  
    # plan_short = plan
    distShort = 0 
    for i in range(0,len(plan_short)-1):
        distShort += numpy.linalg.norm(numpy.array(plan_short[i+1])-numpy.array(plan_short[i])) 
    print "Plan Length: " 
    print distPlan
    ...
    print "Short Length: " 
    print distShort
    old_vertex = plan_short[0]
    if robot.name == 'simple':
        for i in range(1,len(plan_short)):
            new_vertex = plan_short[i]
            planning_env.PlotEdge(old_vertex, new_vertex,4)
