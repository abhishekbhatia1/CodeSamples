...
def pred(numel,full_item_list, pred_mat_combined):
    ...
    partial_avg_list_out = np.zeros([12,12])
    ...
    for i in range(0,numel):
        ...
        for j, item in enumerate(item_list_copy):
            ...
            partial_avg_list_out[i][j] = average_value
    ...
    return prediction, actual_item_list, total_predictions, partial_avg_list_out
    ...
    for nitr in range(0,num_itr):
        ...
        for j in range(1,13):
            ...
            for x, item in enumerate(item_list):
                ...
                pred_mat_combined[x] = pred_mat
                        
            prediction, actual_item_list, total_predictions, confidence = pred(numel, full_item_list, pred_mat_combined)
            bin_list = bin_pred(prediction, old_prediction, active_bins, bin_list)
            old_prediction = prediction
            
            #print "Partial Average List:"
